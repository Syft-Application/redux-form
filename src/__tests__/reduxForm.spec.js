...
const describeReduxForm = (name, structure, combineReducers, setup) => {
  ...
  describe(name, () => {
    ...
    it('values passed to sync validation function should be defined', () => {
      ...
      expect(formRender).toHaveBeenCalled()
      expect(formRender).toHaveBeenCalledTimes(2)
      expect(getIn(validate.mock.calls[0][0], 'foo')).toEqual('bar')
    })

    it('values passed to submit function should be defined', () => {
      const store = makeStore({})
      const onSubmit = jest.fn()
      ...
      expect(getIn(onSubmit.mock.calls[0][0], 'foo')).toEqual('bar')
    })
